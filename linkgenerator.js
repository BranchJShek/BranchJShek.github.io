function createLink() {
  ...
  var range = sheet.getRange(rowIndex, 1, 1, 9) // rowIndex, column, numRows, numCols
  var values = range.getValues();

  var keyInfo = sheet.getRange("I1:J1")
...
function updateLink() {
  ...
  if (branchKey.startsWith("key_live_") && branchSecret) {
    ...
    var branchLinks = sheet.getRange("J3:J300")
...
function createBulk() {
  ...
  const sheet = ss.getSheetByName('Generator')
  
  // select range
  var rowLimit = String(sheet.getRange("J2").getValue()+2)

  // select link data
  var range = sheet.getRange("A3:I" + rowLimit)
  var rows = range.getValues()
  
  // target link column
  var linkColumn = sheet.getRange("I3:I" + rowLimit)
  ...
  var branchKey = sheet.getRange("H1").getValue()  
  var params = {
    'method' : 'POST',
    'contentType' : 'application/json',
